...
# =============================================================================
# ROTAS PARA O ALUNO (DISCIPLINAS E GRADE)
# =============================================================================
...
def disciplinas_aluno():
    ...
    try:
        ...
        cur.execute(
            ...
            WHERE gh.aluno_id = %s ORDER BY d.nome
            """,
            (aluno_id,)
        )
...
def detalhe_disciplina(disciplina_id):
    ...
    disciplina, alunos, atividades = None, [], []
    conn = get_connection()
    cur = conn.cursor()
    try:
...
# ROTA ADICIONADA DE VOLTA PARA CORRIGIR O ERRO
@app.route("/cadastro_atividade")
def cadastro_atividade():
    # Esta rota serve para o aluno criar tarefas pessoais.
    if 'matricula' not in session:
        flash("Você precisa estar logado para criar uma tarefa.", "error")
        return redirect(url_for('index'))
    
    # Passa o user_type para o template saber que é um aluno e uma lista vazia de disciplinas
    return render_template("cadastro_atividade.html", user_type='aluno', disciplinas=[])
