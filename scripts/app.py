...
def menu_aluno():
    ...
    # (Código do menu do aluno que já funcionava)
    aluno = None
    conn = get_connection()
    cur = conn.cursor()
    try:
...
def menu_professor():
    if 'registro' not in session:
        return redirect(url_for('index'))
    # (Código do menu do professor que já funcionava)
    professor = None
    conn = get_connection()
    cur = conn.cursor()
    try:
...
# =============================================================================
# NOVAS ROTAS PARA O ALUNO (DISCIPLINAS E GRADE)
# =============================================================================
...
def disciplinas_aluno():
    ...
    try:
        ...
        if aluno_id_result:
            ...
            # Busca as disciplinas em que o aluno está matriculado através da grade
            cur.execute(
                """
                SELECT d.disciplina_id, d.nome, d.codigo
                FROM academico.disciplinas d
                JOIN academico.grades_disciplinas gd ON d.disciplina_id = gd.disciplina_id
                JOIN academico.grades_horarias gh ON gd.grade_id = gh.grade_id
                WHERE gh.aluno_id = %s
                ORDER BY d.nome
                """,
                (aluno_id,)
    ...
    return render_template("disciplinas_aluno.html", disciplinas=disciplinas)
...
def grade_aluno():
    ...
    # Por enquanto, esta rota apenas exibe a página HTML estática.
    # A lógica para buscar os horários do banco pode ser adicionada aqui no futuro.
    return render_template("grade_aluno.html")


# =============================================================================
# (O restante do código app.py continua aqui)
# Cole todo o resto do seu app.py (rotas de atividades, disciplinas do professor, login, etc.)
...
def salvar_disciplina():
    ...
    nome, codigo, carga_horaria = request.form['nome'], request.form['codigo'], request.form['carga_horaria']
    conn = get_connection()
    cur = conn.cursor()
    try:
        cur.execute("SELECT professor_id FROM academico.professores WHERE numero_registro = %s", (session['registro'],))
...
def salvar_atividade():
    ...
    titulo, descricao, data_entrega, tipo = request.form['titulo'], request.form.get('descricao', ''), request.form['data'], request.form['tipo']
    conn = get_connection()
    cur = conn.cursor()
    try:
        if tipo == 'pessoal':
...
def aluno_atividades():
    ...
    cur = conn.cursor()
    try:
        aluno_id = cur.execute("SELECT aluno_id FROM academico.alunos WHERE matricula = %s", (session['matricula'],))
        aluno_id = cur.fetchone()[0]
